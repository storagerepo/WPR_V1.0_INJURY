...
public class LawyersService {
	...
	public List<LawyersForm> getLawyersList()
	{
		...
		for (Lawyers lawyers : lawyerss) {
			//TODO: Fill the List
			LawyersForm lawyersForm=new LawyersForm(lawyers.getId(), lawyers.getLawyerAdmin().getId(), lawyers.getUsers().getUserId(), lawyers.getFirstName(), lawyers.getLastName(), lawyers.getMiddleName(), lawyers.getGender(), lawyers.getStreetAddress(), lawyers.getCity(), lawyers.getState(), lawyers.getZipcode(), lawyers.getEmailAddress(), lawyers.getPhoneNumber(), lawyers.getNotes(), lawyers.getStatus());
	...
	public LawyersForm getLawyers(Integer getId)
	{
		...
		// Get Mapped Counties
		List<LawyerCountyMappingForm> lawyerCountyMappingForms=lawyerCountyMappingService.getLawyerCountyMappingByLaweyerId(getId);
		...
		for (LawyerCountyMappingForm lawyerCountyMappingForms1: lawyerCountyMappingForms) {
			if(lawyerCountyMappingForms1.getCountyId()!=null){
				System.out.println("Layer County Id"+lawyerCountyMappingForms1.getCountyId());
	...
	public int mergeLawyers(LawyersForm lawyersForm)
	{
		...
		Lawyers lawyers=new Lawyers(lawyerAdmin, users, lawyersForm.getFirstName(), lawyersForm.getLastName(), lawyersForm.getMiddleName(), lawyersForm.getGender(), lawyersForm.getStreetAddress(), lawyersForm.getCity(), lawyersForm.getState(), lawyersForm.getZipcode(), lawyersForm.getEmailAddress(), lawyersForm.getPhoneNumber(), lawyersForm.getNotes(), 1, null);
	...
	public int saveLawyers(LawyersForm lawyersForm)
	{
		...
		for (Integer countyId : countyMapped) {
			...
			LawyerCountyMapping lawyerCountyMapping=new LawyerCountyMapping(lawyers, county);
	...
	public int updateLawyers(LawyersForm lawyersForm)
	{
		...
		Integer[] countyMapped=lawyersForm.getCountyId();
		lawyerCountyMappingService.deleteLawyerCountyMapping(countyMapped, lawyersForm.getId());
		// Add a New County List
		List<Integer> newlyAddedCounty=lawyerCountyMappingService.getNewlyAddedCountyId(countyMapped, lawyersForm.getId());
	...
	public Integer getNumberOfLawyers(Integer lawyerAdminId){
		...
		numberOfLawyerAdmin=this.getLawyersListByLawyerAdmin(lawyerAdminId).size();
	...
	public List<PatientsForm> getPatientsByLawyer(Integer lawyerId){
		...
		// Get Mapped Counties
		List<LawyerCountyMappingForm> lawyerCountyMappingForms=lawyerCountyMappingService.getLawyerCountyMappingByLaweyerId(lawyerId);
		for (LawyerCountyMappingForm lawyerCountyMappingForm : lawyerCountyMappingForms) {
			County county=countyDAO.get(lawyerCountyMappingForm.getCountyId());
		...
		for (CountyForm countyForms1 : countyForms) {
			for (PatientsForm patientsForm : patientsForms) {
				if(countyForms1.getName().equals(patientsForm.getCountry().split("\\s+")[0])){
