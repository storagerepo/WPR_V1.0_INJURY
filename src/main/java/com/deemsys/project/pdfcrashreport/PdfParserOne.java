	...
	public PDFCrashReportJson parsePdfFromFile(File file) throws IOException {
		...
		try {
			...
			for (int j = Integer.parseInt(reportFirstPageForm.getNumberOfUnits()) + 2 + IncrementPage; j <= reader
				...
				if (!pdfText.equals("") && ((pdfText.equals("OH") || pdfText.equals("WA"))
						|| (!pdfText.matches("[0-9]+") && pdfText.length() == 2))) {

					occupantsArray = InjuryConstants.MOTORIST_PAGE_COORDINATES;
				} else {
					occupantsArray = InjuryConstants.OCCUPANT_PAGE_COORDINATES;
				}
