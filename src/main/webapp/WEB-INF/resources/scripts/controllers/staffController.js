...
adminApp.controller('ShowStaffController', function($scope,$http,$location,$state) {
    ...
    $http.get("http://localhost:8086/Injury/Staff/getAllStaffs.json").success( function(response) {
    ...
    $scope.deleteStaff=function(id)
	  {
		  ...
		  $http({
			  method : "POST",
			  url : "http://localhost:8086/Injury/Staff/deleteStaff.json?id="+id,
...
adminApp.controller('SaveStaffController', function($scope,$http,$location) {
	...
	$scope.save=function()
	{
		...
		$http.post("http://localhost:8086/Injury/Staff/saveUpdateStaff",$scope.staff)
			.success(function(response)
					{
...
adminApp.controller('EditStaffController', function($scope,$http,$location,$stateParams) {
	...
	$scope.options=false;
	$http.get('http://localhost:8086/Injury/Staff/getStaff.json?id='+ $stateParams.id).success( function(response) {
	    $scope.staff=response.staffForm;
});

	  $scope.update=function(){
		  
	  $http.post('http://localhost:8086/Injury/Staff/saveUpdateStaff.json', $scope.staff).success(function (status) {
